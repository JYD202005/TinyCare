...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    if (error) {
      ...
      // Reclamar invitaciones pendientes si hay
  ...
  return (
    <View style={s.root}>
      {ToastComponent}
      <View style={s.header}>
        <Ionicons name="cloud-done" size={48} color={TC.vitalHeart} />
        <Text style={s.title}>Iniciar Sesión</Text>
        <Text style={s.subtitle}>Accede a los datos de TinyCare en la nube</Text>
      </View>
      ...
      <View style={s.form}>
        ...
        {loading ? (
          ...
          <View style={s.buttons}>
            ...
            <TouchableOpacity style={s.btnGhost} onPress={() => router.push('/register')}>
              <Text style={s.btnGhostText}>¿No tienes cuenta? Registrate</Text>
            </TouchableOpacity>

            <TouchableOpacity style={s.btnGhost} onPress={() => router.back()}>
              <Text style={[s.btnGhostText, { color: '#9CA3AF' }]}>Continuar Offline (Local)</Text>
            </TouchableOpacity>
          </View>
        )}
      </View>
...
const s = StyleSheet.create({
  ...
  btnGhostText: { color: TC.vitalHeart, fontSize: 14, fontWeight: '600' }
});
